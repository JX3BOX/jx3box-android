...
class MutlColorLoadingView : View {
    ...
    //存储宽高
    private lateinit var mLayer: RectF
    ...
    //第一回合所占时间比例
    private var rateOfFirstRound = 0.45f
    ...
    private fun initAll() {
        ...
        mLoadingAnimator.repeatCount = Animation.INFINITE
        mLoadingAnimator.interpolator = MutlColorInterpolator(rateOfFirstRound)
    ...
    private var mUpdateListener = AnimatorUpdateListener {
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        mArcPath.reset()
//        val len = w.coerceAtMost(h).toFloat()
    ...
    private fun drawSearch(canvas: Canvas) {
        ...
        if (progress >= stagemMid) {
            ...
            mMeasure.getSegment(
                start,
                (1 / 2f * progress * (3 - progress) * mMeasureLength),
                path,
                true
            )
    ...
    private fun drawSecondSegment(canvas: Canvas, curProgress: Float) {
        //第二回合 蓝色生命周期只有 0.7倍
        var mCurProgress = curProgress
        mCurProgress = conveterToRealProgress(mCurProgress, 0.6f)
        mPaint.color = secondColor
        val start = 0f
        val end = mCurProgress * mMeasureLength
        val path = Path()
        mMeasure.getSegment(start, end, path, true)
        canvas.drawPath(path, mPaint)
    }
    ...
    private fun drawFirstSegment(canvas: Canvas, curProgress: Float) {
        ...
        // 白色loading 占第二回合的 0.6 个生命周期
        mCurProgress = conveterToRealProgress(mCurProgress, 0.6f)
        val start = mCurProgress * mMeasureLength
     ...
     */
    class MutlColorInterpolator(private var rate: Float) : Interpolator {
